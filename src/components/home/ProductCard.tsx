...
import { Heart, MapPin, Leaf, CheckCircle2, Box, Sparkles, Truck } from 'lucide-react';
import { useAuth } from '@/contexts/AuthContext';
import { useWishlist } from '@/contexts/WishlistContext';
import { Product } from '@/lib/types';
import { calculateDistanceKm, calculateOrderImpact, formatCO2Tons } from '@/lib/carbon-calculator';
...
export default function ProductCard({ product, onClick }: ProductCardProps) {
  ...
  const transportAtMoq = calculateOrderImpact(product, product.moq, distanceKm).transportEmissions;
        ...
        <div className="mt-auto pt-2 space-y-2">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-1.5 flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[11px] font-semibold text-slate-600">Transport at MOQ</span>
            </div>
            <span className="text-[11px] font-bold text-slate-800">{formatCO2Tons(transportAtMoq)}</span>
          </div>
