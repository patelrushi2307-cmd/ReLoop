    ...
    >
      ...
      <div className="grid gap-x-8 gap-y-12 md:grid-cols-4">
        ...
        <motion.div
          ...
          className="flex flex-col gap-4 border-t border-orange pt-5"
        >
          <div className="flex items-baseline gap-2 text-[clamp(2.6rem,5.4vw,4.6rem)] font-medium leading-[0.9] tracking-[-0.04em] text-orange">
          ...
          <div className="flex gap-[3px]" aria-hidden>
            {Array.from({ length: 28 }).map((_, i) => (
              <motion.span
                ...
                className={`h-4 w-[3px] ${i < 20 ? "bg-orange" : "bg-[var(--fg)]"}`}
              />
            ))}
          </div>
