...
import { useReveal } from "@/components/motion/RevealProvider";
...
export function Hero() {
  ...
  return (
    <section
      id="top"
      ref={ref}
      className="relative isolate flex min-h-[calc(100svh-6.75rem)] flex-col justify-end overflow-hidden px-4 pb-8 pt-10 md:px-8 md:pb-10"
    >
      {/* blur layers */}
      <div className="pointer-events-none absolute inset-0 -z-10" aria-hidden>
        <motion.div
          style={{ y: still ? 0 : blobA }}
          className="absolute -left-[10%] top-[6%] h-[52vh] w-[52vh] rounded-full bg-blue/18 blur-[110px]"
        />
        <motion.div
          style={{ y: still ? 0 : blobB }}
          className="absolute right-[4%] top-[26%] h-[44vh] w-[44vh] rounded-full bg-orange/16 blur-[120px]"
        />
        <motion.div
          style={{ y: still ? 0 : blobC }}
          className="absolute bottom-[-14%] left-[34%] h-[46vh] w-[62vh] rounded-full bg-blue-deep/12 blur-[130px]"
        />
      </div>
        ...
        <div className="mt-8 flex flex-col gap-6 border-t border-[var(--line)] pt-6 md:flex-row md:items-end md:justify-between">
          ...
          >
            <a
              ...
              className="group flex h-12 items-center gap-3 rounded-full bg-[var(--fg)] pl-6 pr-5 text-[14px] font-medium text-[var(--bg)]"
            >
            ...
            <a
              ...
              className="flex h-12 items-center rounded-full border border-[var(--line)] px-6 text-[14px] font-medium text-[var(--fg)] transition-colors hover:border-[var(--fg)]"
            >
        ...
        >
          {META.map((m) => (
            <div key={m.k} className="flex flex-col gap-1.5">
              ...
              <dd className="tnum text-[14px] text-[var(--fg)]">{m.v}</dd>
            </div>
          ))}
