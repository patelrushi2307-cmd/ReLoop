...
export class CarbonController {
  ...
  /**
   * Compares the same haul across vehicle classes (POST /carbon/compare).
   */
  async compare(req: Request, res: Response, next: NextFunction) {
    try {
      const { materialCategory, materialSubtype, massKg, distanceKm, loadFactor } = req.body;
      ...
      if (!materialCategory || typeof massKg !== 'number') {
        return res.status(400).json({
          ...
          error: {
            code: 'INVALID_INPUT',
            message: 'materialCategory and massKg (number) are required for a vehicle comparison',
            fields: {},
          },
        });
      }

      const data = carbonService.compareVehicles({
        materialCategory,
        materialSubtype,
        massKg,
        distanceKm: Number(distanceKm) || 0,
